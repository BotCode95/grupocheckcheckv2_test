...
export const Testimonials = () => {
	...
	const [showModalTestimonials, setShowModalTestimonials] =
		useState<IModalTestimonialOptions>({
			open: false,
			action: null,
			data: undefined,
		})
	...
	const handleDeleteTestimonial = () => {
		const newTestimonials = text?.testimonials.filter(
			(el) => !(el.author === showDeleteModal.data?.author)
		)
			...
			{visibleRows?.length > 0 && (
				<>
						...
						<Table style={{ color: 'white' }}>
							...
							<TableBody>
								{visibleRows?.map((testimonial) => {
									return (
										<TableRow key={testimonial?.author}>
											<StyledTableCell>
												<div style={{}}>
													<img
											...
											<StyledTableCell>{testimonial?.author}</StyledTableCell>
											<StyledTableCell>
												{testimonial?.title || '-'}
											</StyledTableCell>
					...
					<TablePagination
						...
						style={{ color: 'white' }}
						component="div"
						count={text?.testimonials?.length}
